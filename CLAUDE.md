...
Maestro is a Python CLI app that auto-plays MIDI songs on in-game pianos by simulating keyboard presses. Supports Heartopia, Where Winds Meet, and Once Human. The GUI opens automatically on startup. Users select a MIDI file, and after a 3-second countdown, the app plays it by pressing the appropriate keys.
...
Maestro (main.py) - coordinates everything, Qt event loop on main thread
    ...
    │       ├── keymap_once_human.py - Once Human mapping (modifier-based octave switching)
    │       ├── key_layout.py - KeyLayout enum (Heartopia), WwmLayout enum (WWM)
    │       └── game_mode.py - GameMode enum (Heartopia, Where Winds Meet, Once Human)
...
- `src/maestro/keymap_wwm.py` - Where Winds Meet key mapping: 36-key (Shift for C#/F#/G#, Ctrl for Eb/Bb) and 21-key (naturals only)
- `src/maestro/keymap_once_human.py` - Once Human key mapping: single-octave keys (Q-U naturals, 2/3/5/6/7 accidentals) with Shift=high octave, Ctrl=low octave
...
- `src/maestro/key_layout.py` - KeyLayout enum for Heartopia (KEYS_22, KEYS_15_DOUBLE, KEYS_15_TRIPLE, DRUMS, XYLOPHONE), WwmLayout enum for WWM (KEYS_36, KEYS_21)
- `src/maestro/game_mode.py` - GameMode enum for game selection (Heartopia, Where Winds Meet, Once Human)
...
- **Keys layout dropdown**: Heartopia: 22-key, 15-key Double Row, 15-key Triple Row, Conga/Cajon (8-key), Xylophone (8-key); WWM: 36-key (Full), 21-key (Naturals); Once Human: hidden (one fixed layout). Dropdown repopulates on game mode change.
...
- **DirectInput for WWM and Once Human**: Where Winds Meet and Once Human require DirectInput; uses pydirectinput (Windows-only, falls back to pynput elsewhere). Controlled by `_DIRECTINPUT_MODES` frozenset in player.py. WWM: 36-key layout uses Shift (C#, F#, G#) and Ctrl (Eb, Bb) modifiers; 21-key layout is naturals only. Once Human: single-octave keys with Shift=high octave, Ctrl=low octave.
- **Once Human piano**: Single-octave keyboard layout with modifier-based octave switching. White keys Q-U (naturals), number keys 2/3/5/6/7 (accidentals). Shift raises to octave 5, Ctrl lowers to octave 3, no modifier for octave 4. All 36 chromatic notes (C3-B5) directly playable. No layout variants or sharp handling needed.
