...
class TestNoteMappings36Key:
    ...
    @pytest.mark.parametrize(
        "note,expected",
        [
            (48, ("z", 48, None)),  # C3 natural
            (60, ("a", 60, None)),  # C4 natural
            (72, ("q", 72, None)),  # C5 natural
            # Shift accidentals (C#, F#, G#)
            (49, ("z", 49, Key.shift)),  # C#3 → Shift+Z
            (61, ("a", 61, Key.shift)),  # C#4 → Shift+A
            (73, ("q", 73, Key.shift)),  # C#5 → Shift+Q
            (66, ("f", 66, Key.shift)),  # F#4 → Shift+F
            (68, ("g", 68, Key.shift)),  # G#4 → Shift+G
            # Ctrl accidentals (Eb, Bb)
            (51, ("c", 51, Key.ctrl_l)),  # Eb3 → Ctrl+C
            (63, ("d", 63, Key.ctrl_l)),  # Eb4 → Ctrl+D
            (75, ("e", 75, Key.ctrl_l)),  # Eb5 → Ctrl+E
            (58, ("m", 58, Key.ctrl_l)),  # Bb3 → Ctrl+M
            (70, ("j", 70, Key.ctrl_l)),  # Bb4 → Ctrl+J
            (82, ("u", 82, Key.ctrl_l)),  # Bb5 → Ctrl+U
        ],
    )
...
class TestTranspose36Key:
    ...
    def test_transpose_below_into_range(self):
        assert midi_note_to_key_wwm(36, transpose=True) == ("z", 48, None)

    def test_transpose_above_into_range(self):
        assert midi_note_to_key_wwm(84, transpose=True) == ("q", 72, None)

    def test_transpose_does_not_affect_in_range(self):
        assert midi_note_to_key_wwm(60, transpose=True) == ("a", 60, None)

    def test_transpose_preserves_shift_modifier(self):
        assert midi_note_to_key_wwm(37, transpose=True) == ("z", 49, Key.shift)

    def test_transpose_preserves_ctrl_modifier(self):
        # MIDI 39 = Eb2, transposes up to Eb3 (MIDI 51)
        assert midi_note_to_key_wwm(39, transpose=True) == ("c", 51, Key.ctrl_l)
...
class TestNoteMappings21Key:
    ...
    @pytest.mark.parametrize(
        "note,expected_key,snapped",
        [
            (49, "z", 48),  # C#3 → snap to C3
            (51, "x", 50),  # Eb3 → snap to D3
            (66, "f", 65),  # F#4 → snap to F4
            (68, "g", 67),  # G#4 → snap to G4
            (70, "h", 69),  # Bb4 → snap to A4
        ],
    )
    def test_sharp_snap_maps_to_natural(self, note, expected_key, snapped):
        result = midi_note_to_key_wwm_21(note, sharp_handling="snap")
        assert result == (expected_key, snapped, None)
...
class TestTranspose21Key:
    ...
    def test_transpose_below(self):
        assert midi_note_to_key_wwm_21(36, transpose=True) == ("z", 48, None)

    def test_transpose_above(self):
        assert midi_note_to_key_wwm_21(84, transpose=True) == ("q", 72, None)
    ...
    def test_transpose_sharp_snap(self):
        ...
        assert midi_note_to_key_wwm_21(37, transpose=True, sharp_handling="snap") == ("z", 48, None)
